...
const maxDailyPlays = 10; // 設定每天上限次數
...
const checkPlayLimit = () => {
 const today = new Date().toLocaleDateString(); // 取得今天日期字串 (例如 "2024/5/20")
 const gameRecord = JSON.parse(localStorage.getItem('snake_game_record') || '{}');

 // 如果紀錄裡的日期不是今天，重置次數
 if (gameRecord.date !== today) {
 const newRecord = { date: today, count: 0 };
 localStorage.setItem('snake_game_record', JSON.stringify(newRecord));
 remainingPlays.value = maxDailyPlays;
 return true;
 }

 // 如果是今天，檢查次數
 remainingPlays.value = maxDailyPlays - gameRecord.count;
 return gameRecord.count < maxDailyPlays;
};
...
const handleStartGame = () => {
 ...
 icon: 'error',
 title: '次數已用盡',
 text: `今天已經玩過 ${maxDailyPlays} 次囉，請明天再來！`,
 ...
 const gameRecord = JSON.parse(localStorage.getItem('snake_game_record'));
 gameRecord.count += 1;
 localStorage.setItem('snake_game_record', JSON.stringify(gameRecord));
 remainingPlays.value = maxDailyPlays - gameRecord.count;

 // 這裡執行你原本啟動遊戲的程式碼 (例如 startSnakeGame())
 console.log("遊戲開始！");
};

onMounted(() => {
 checkPlayLimit(); // 頁面載入時先更新剩餘次數顯示
});
