...
CREATE TABLE merchant (
    merchantId INT IDENTITY(1,1) PRIMARY KEY,          -- PK

    merchantName NVARCHAR(50) NOT NULL,                -- 商家名稱
    merchantPhone NVARCHAR(20),                        -- 電話
    merchantEmail NVARCHAR(50) NULL,                   -- Email
    merchantAddress NVARCHAR(200) NOT NULL,            -- 地址
...
CREATE TABLE discount (
    ...
    couponDescription NVARCHAR(1000),                  -- 優惠內容
    pointsRequired INT NOT NULL,                       -- 點數需求

    startDate DATE,                                    -- 生效日期
    endDate DATE,                                      -- 結束日期

    merchantId INT,                                    -- FK 商家ID
    couponStatus INT NOT NULL,                         -- 上下架
    createdTime DATETIME2 DEFAULT SYSDATETIME(),       -- 建立時間
...
INSERT INTO merchant (merchantName, merchantPhone, merchantEmail, merchantAddress, merchantStatus)
VALUES
...
INSERT INTO discount (couponDescription, pointsRequired, startDate, endDate, merchantId, couponStatus)
VALUES
...
Select * from merchant;
INSERT INTO discount (couponName,couponDescription, pointsRequired, startDate, endDate, merchantId, couponStatus,couponImg)
VALUES
...
CREATE TABLE member ( 
memId INT IDENTITY(1,1) PRIMARY KEY,                       --會員ID
memUsername VARCHAR(50) NOT NULL UNIQUE,                   --帳號
memPassword VARCHAR(100) NOT NULL,                         --密碼
memName NVARCHAR(50) NOT NULL,                             --姓名
memEmail VARCHAR(50) NOT NULL UNIQUE,                      --信箱
memPhone VARCHAR(20) NOT NULL,                             --手機號碼
memStatus INT NOT NULL DEFAULT 1,                          --會員狀態
createdAt DATETIME2 NOT NULL DEFAULT SYSDATETIME(),        --建立時間
updatedAt DATETIME2 NOT NULL DEFAULT SYSDATETIME(),        --更新時間
memPoints INT NOT NULL DEFAULT 0,                          --點數累計
memViolation INT NOT NULL DEFAULT 0,                       --違規紀錄
memLevel INT NOT NULL DEFAULT 1,                           --會員等級
memInvoice VARCHAR(20) NULL                                --發票載具
);


CREATE TABLE admin ( 
admId INT IDENTITY(1,1) PRIMARY KEY,                       --管理員ID
admUsername VARCHAR(50) NOT NULL UNIQUE,                   --帳號
admPassword VARCHAR(100) NOT NULL,                         --密碼
admName NVARCHAR(50) NOT NULL,                             --姓名
admEmail VARCHAR(50) NOT NULL,                             --信箱
admRole INT NOT NULL DEFAULT 1,                            --管理員權限
createdAt DATETIME2 NOT NULL DEFAULT SYSDATETIME(),        --建立時間
updatedAt DATETIME2 NOT NULL DEFAULT SYSDATETIME()         --更新時間
);

INSERT INTO member (memUsername, memPassword, memName, memEmail, memPhone, 
...
INSERT INTO admin (admUsername, admPassword, admName, admEmail, admRole)
VALUES
...
INSERT INTO renting_Spot (spotCode, spotName, spotAddress, spotStatus, merchantId, createdAt, updatedAt)
VALUES 
...
--seats
INSERT INTO seats (seatsName, seatsType, seatsStatus, spotId, updatedAt, serialNumber, createdAt)
VALUES 
...
(N'按摩椅-H01',  N'T椅', N'空閒', 8, SYSDATETIME(), 'SN-2025012', DEFAULT), -- 關聯到已停用的站點，測試邏輯
(N'按摩椅-I01',  N'H椅', N'空閒', 9, SYSDATETIME(), 'SN-2025013', DEFAULT),
(N'按摩椅-J01',  N'T椅', N'空閒', 10, SYSDATETIME(), 'SN-2025014', DEFAULT),
(N'備用設備-Z99',  N'H椅', N'庫存', NULL, SYSDATETIME(), 'SN-2025999', DEFAULT); -- 測試 spotId 為 NULL 的情況
...
CREATE TABLE recRent (    
    ...
    memId INT NOT NULL,       　  --  外鍵：Member ID (可為 NULL)
    couponId INT  NULL,      　   --  外鍵：使用的優惠方案
    seatsId VARCHAR(10) NOT NULL,      　 --  外鍵：便攜座椅編號
    spotIdRent INT NOT NULL,   　  --  外鍵：租用點機台 ID (必須與 spot 表格的 spotId 類型一致)
    spotIdReturn INT NULL,      　 --  外鍵：歸還點機台 ID (可為 NULL)
    recRentDT2 DATETIME2(0) NOT NULL,　　 -- 租用日期時間    
    recReturnDT2 DATETIME2(0) NULL,　　　 -- 歸還日期時間 (未歸還時為 NULL)    
    recUsageDT2 DECIMAL NULL,　　　　-- 使用時間 DATETIME2     
    recStatus NVARCHAR(15) NULL,　　　　　-- 訂單狀態
    recPrice INT NULL,         　　　　　 -- 原始價格
    recRequestPay INT NULL,     　　　　　-- 請款金額
    recPayment INT NULL,　　　　　　　　　-- 付款金額
    recPayBy VARCHAR(16) NULL,　　　　　　-- 付款方式
    recInvoice  VARCHAR(16) NULL,　　　 　-- 發票號碼
    recCarrier  VARCHAR(16) NULL,　　 　　-- 發票載具　
    recViolatInt  INT NOT NULL,　　　　 　-- 違規類型
    recNote NVARCHAR(255) NULL,　　　　 　-- 備註
    -- 6. 設定約束 (Constraints)
    CONSTRAINT PK_recRent_seq PRIMARY KEY (recSeqId),      -- 技術主鍵
    CONSTRAINT UK_recRent_recId UNIQUE (recId),            -- 業務主鍵 (R00001)
